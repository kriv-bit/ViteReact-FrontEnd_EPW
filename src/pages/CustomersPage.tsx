...
import type { UpdateCustomerDto } from "../api/customers";
...
export default function CustomersPage() {
  ...
  async function saveEdit(id: number) {
    ...
    if (editFullName.trim()) dto.fullName = editFullName;
    if (editEmail.trim()) dto.email = editEmail;

    await updateMut.mutateAsync({ id, dto });
    cancelEdit();
  }
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 space-y-4">
        ...
        <div className="rounded-xl border bg-white">
                    ...
                    {editingId === c.id ? (
                      <>
                        ...
                        <td className="p-3 space-x-2">
                          <button
                            className="rounded-md bg-green-600 px-2 py-1 text-white hover:bg-green-700 disabled:opacity-50"
          ...
          {updateMut.isError && (
            <div className="p-4">
              <p className="text-sm text-red-600">
                Error editando: {String(updateMut.error)}
              </p>
            </div>
          )}
        </div>
